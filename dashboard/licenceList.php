...
$pageName = "Driving Licence Record"; // Replace this with the actual page name
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <input type="search" autocomplete="off" name="search" class="form-control border-danger"
                                    placeholder="Search By:- DL Number / Name / Request ID"
                                    autofocus />
                            </div>
                        </div>
                        <div class="col-1">
                ...
                <div id="demo_info" class="p">
                    <table id="example" class="datatables-basic table border-top" style="width:100%">
                        ...
                        <tbody>
                            ...
                            $stmt = $conn->prepare("select * from printRecords WHERE userId = ? AND print_type ='Driving Licence' ORDER BY `id` DESC");
                            ...
                            if (isset($_POST['daterange'])) {
                                ...
                                $stmt = $conn->prepare("SELECT * FROM printRecords WHERE date BETWEEN :fromdate AND :todate AND userId = :userId AND print_type ='Driving Licence' ORDER BY `id` DESC");
                                $stmt->execute([
                                    'fromdate' => $fromdate,
                                    'todate' => $todate,
                                ...
                                $stmt = $conn->prepare("SELECT * FROM printRecords WHERE userId = :userId AND print_type ='Driving Licence' ORDER BY `id` DESC");
                                $stmt->execute([
                                    'userId' => getUsersInfo('id')
                                ]);
                            }
                            
                            $stmt = $conn->prepare("select * from printRecords WHERE date between '".$fromdate."' AND '".$todate."' AND userId=? AND print_type ='Driving Licence' ORDER BY `id` DESC");
                            $stmt->execute([getUsersInfo('id')]);
                            ...
                            $stmt = $conn->prepare("select * from printRecords WHERE idNumber LIKE '{$search}%' OR print_type LIKE '{$search}%' OR name LIKE '{$search}%' 
                             AND userId=? AND print_type ='Driving Licence' ORDER BY `id` DESC");
                            $stmt->execute([getUsersInfo('id')]);
                            }
                            $sl=1;
                            while($row=$stmt->fetch()) {
                            $printdata = "<div class='dropdown-menu dropdown-menu-end'>
                                    <a href='../printManagement/licenceHd?token=" . base64_encode($row['id']) . "&access_token=" . base64_encode($row['idNumber']) . "' target='_blank' class='dropdown-item' target='_blanck'><i class='bx bx-printer me-1'></i> Licence</a>
                                  </div>";
